class WhereV1 extends React.Component {
	...
	callSubmitAddressApi(){
        ...
        let url=this.props.link+'/api/v3/submit_address';
            ...
            .then((response)=>{
            	...
            	if (parseInt(response.status) !== 202) {
                	...
                	response.json().then((object) =>{
                		...
                		packageInfos = packageArrangement(object['packages']);
                		let recommendationGalleryIndex = this.findIndexOfHour(this.state.recommendationHour,packageInfos.galleryEnglishHours);
                    	let recommendationMemoryIndex = this.findIndexOfHour(this.state.recommendationHour,packageInfos.memoryEnglishHours);
	...
	render () {
		...
		let mapHeight=(mobileDisplay)?window.screen.availHeight*0.67:'400px';
					...
					<div className="col-sm-6 filter-checkboxes-container text-left" style={{padding: '0px'}}>
						...
						<a onClick={this.handleMyLocationCheckBoxChange}>
							<div className={"filter-mylocation-box "+((this.state.myLocationFilterSelected)? 'selected':'')}>
								...
								/>
								<label className="my-location-check-label" htmlFor="my-location-check" style={{marginRight:'5px'}}>در محل</label>
							</div>
						</a>
						...
						<a onClick={this.handleStudioCheckBoxChange}>
							<div className={"filter-studio-box "+((this.state.showStudios)? 'selected':'')}>
								...
								/>
								<label className="studios-check-label" htmlFor="studio-locations-check" style={{marginRight:'5px'}}>آتلیه ها</label>
							</div>
						</a>
