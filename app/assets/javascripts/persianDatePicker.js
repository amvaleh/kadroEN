...
(function ($) {
    ...
    var persianDatepicker  = (function () {
        function persianDatepicker(element, userOptions) {
            ...
            $('#date-picker').append(self.calendar);
            self.render();
        }
        ...
        persianDatepicker.prototype = {
            ...
            showDate: function (el, jDate, gDate, showGdate) {
                var pb = new persianDate();
                var self = this;
                var year = self.persianDate.parse(jDate).year;
                var week = self.persianDate.parse(jDate).day;
                var day = self.persianDate.parse(jDate).date;
                console.log(self.persianDate.parse(jDate));
                var dateComplete = pb.dowTitle[week] + " - " + jdf.toPersianNums(day) + " - " + self.options.months[self.persianDate.month - 1]  +  " - " + jdf.toPersianNums(year);

                jDate    = self.persianDate.parse(jDate).toString("YYYY/NM/DD");
                ...
                if (el.is('input:text')) {
                    ...
                    if (showGdate) {
                        ...
                        //console.log(jDate);
                        var location = $('#time-picker').find('header');
                        location.html(dateComplete);
                        location.attr('data-gdate', gDate);
                        location.attr('data-jdate', jDate);
                    }
                }
