module ShareControls
  class GetGalleriesForRequestSentToUser
    include Interactor
    QUERY = <<-SQL
select g.* , u.full_name as user_name , CONCAT(ph.first_name, ' ' , ph.last_name) as ph_name , u.mobile_number as user_mobile_number
...
where sh.request_sent_to_user = false and sh.permit = false and g.id IS NOT NULL
GROUP BY g.id) g
